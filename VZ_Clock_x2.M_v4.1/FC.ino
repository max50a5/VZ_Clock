bool loadConfig() {                                                // Завантаження даних збереженних в файлі config.json
  ...
  DynamicJsonDocument doc(4546);                                   // Резервуємо память для json обекту буфер може розти по мірі необхідності переважно для ESP8266 
  ...
  sendtemp = doc["sendtemp"];
  sendtempul = doc["sendtempul"];
  sendother = doc["sendother"];
  sendhum = doc["sendhum"];
  sendpress = doc["sendpress"];
  ...
  snprintf(mqtt_sub1, 49, "%s", (doc["mqtt_sub1"].as<String>()).c_str());
  snprintf(mqtt_sub2, 49, "%s", (doc["mqtt_sub2"].as<String>()).c_str());
  snprintf(mqtt_sub3, 49, "%s", (doc["mqtt_sub3"].as<String>()).c_str());
...
bool saveConfig() {
  ...
  doc["sendtemp"] = sendtemp;
  doc["sendtempul"] = sendtempul;
  doc["sendother"] = sendother;
  doc["sendhum"] = sendhum;
  doc["sendpress"] = sendpress;
