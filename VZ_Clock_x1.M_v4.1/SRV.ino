...
void handle_ConfigSetupJson() {
  ...
  json += "\",\"sensorDom\":\"";
  json += (sensorDom==0?"NONE":sensorDom==1?"DS18B20":sensorDom==2?"SI7021":sensorDom==3?"BMP280":sensorDom==4?"BME280":sensorDom==6?"MQTT1":sensorDom==7?"MQTT2":sensorDom==8?"MQTT3":"NONE");
  json += "\",\"corrTempD\":\"";
  json += corrTempD;
  json += "\",\"Td\":\"";
  json += t0;
...
void handle_weather() {
  ...
  if(printCom) {
    ...
    Serial.println(", Key0: " + weatherKey0 + ", City ID0: " + cityID0 + ", City ID1: " + cityID1 + ", weatherLang: " + weatherLang);
    ...
    Serial.println("outtimeScrollStart: " + String(timeScrollStart) + ", timeScrollStop: " + String(timeScrollStop) + ", timeScrollSpeed: " + String(timeScrollSpeed));
  }
  saveConfig();
  lang();
...
void handle_setup(){
  ...
  if(server.arg("buzzerOnOffku")!="") buzzerOnOffku = server.arg("buzzerOnOffku").toInt();
  if(server.arg("buzzerOnOff")!="") buzzerOnOff = server.arg("buzzerOnOff").toInt();
...
void handle_mqtt_ust() {
  ...
  if(server.arg("mqtt_sub1")!="") snprintf(mqtt_sub1, 49, "%s", server.arg("mqtt_sub1").c_str());
  if(server.arg("mqtt_sub2")!="") snprintf(mqtt_sub2, 49, "%s", server.arg("mqtt_sub2").c_str());
  if(server.arg("mqtt_sub3")!="") snprintf(mqtt_sub3, 49, "%s", server.arg("mqtt_sub3").c_str());
